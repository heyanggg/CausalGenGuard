...
2. Train a short SmartGuard backbone run:
   PYTHONPATH=src /home/heyang/miniconda3/envs/smartguard_env/bin/python -m causal_gen_guard.training.train_backbone --input-jsonl outputs/processed/fr_sequences_canonical.jsonl --epochs 1 --batch-size 32 --hidden-dim 64 --nhead 4 --num-layers 1 --checkpoint-path outputs/checkpoints/smartguard_backbone_fr.pt --loss-vector-output outputs/results/loss_vector_fr.npy
